...
**[DATA.md](DATA.md)** — it covers running the master pull
...
`makeTables.m` builds the LaTeX tables and writes them to `tables/`. Run the
`Table A1a ...` block for the baseline (Model 1) decomposition and the
`Table A1b ...` block for the convenience-yield (Model 2) decomposition.
...
instead of by hand, see **[DATA.md](DATA.md)**.
