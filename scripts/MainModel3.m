...
disp('mean Infl')
disp(nanmean(nanmean(Y(T0pre:T1pre,Nc+1:2*Nc))))

disp('mean Ltir')
disp(nanmean(nanmean(Y(T0pre:T1pre,2*Nc+1:3*Nc))))

disp('mean Dcons')
disp(nanmean(nanmean(Y(T0pre:T1pre,3*Nc+1:4*Nc))))

disp('mean Baa us')
disp(nanmean(nanmean(Y(T0pre:T1pre,4*Nc+1))))
...
disp('std Infl')
disp(nanmean(nanstd(Y(T0pre:T1pre,Nc+1:2*Nc))))

disp('std Ltir')
disp(nanmean(nanstd(Y(T0pre:T1pre,2*Nc+1:3*Nc))))

disp('std Dcons')
disp(nanmean(nanstd(Y(T0pre:T1pre,3*Nc+1:4*Nc))))

disp('std Baa')
disp(nanmean(nanstd(Y(T0pre:T1pre,4*Nc+1))))
...
Ctr =[
    ...
    1       0       0         0       0       1%     Dcons_au...
    1       0       0         0       0       1%     Dcons_be...
    1       0       0         0       0       1%     Dcons_fi...
    1       0       0         0       0       1%     Dcons_ie...
    1       0       0         0       0       1%     Dcons_nl...
    1       0       0         0       0       1%     Dcons_no...
    1       0       0         0       0       1%     Dcons_ch...
    1       0       0         0       0       1%     Dcons_se...
    1       0       0         0       0       1%     Dcons_es...
    1       0       0         0       0       1%     Dcons_pt...
    1       0       0         0       0       1%     Dcons_dk...
    1       1       1         0       1       0%     Baa_us
    ];
...
Cadd1(4*Nc+1, 1) = 0; %no convenience yield of Baa (Baa is the LAST row = 4*Nc+1, after Stir/Infl/Ltir/Dcons)
...
Cadd2(4*Nc+1, 1) = 1;   % Baa (last row = 4*Nc+1) is nominal -> loads on US idio inflation. FIX: was 3*Nc+1 (=Dcons_us) in the 18-country port; original used the Baa row (4*Nc+1)
...
Cadd3(4*Nc+1, 1) = 1;   % Baa (last row = 4*Nc+1) loads on US idio term spread. FIX: was 3*Nc+1 (=Dcons_us)

%Adding country specific trends to consumption
% Cadd4                 =    zeros(n,7);
% Cadd4(22:28,1:7)      =    eye(7);  % Ltir
Cadd4 = zeros(n, Nc);
Cadd4(3*Nc+1:4*Nc, 1:Nc) = eye(Nc);
...
Psi =       (2*[  ones(1,Nc)   2*ones(1,Nc)  ones(1,Nc)   2*ones(1,Nc)   1  ]).^2;
...
for jm = 1:Ndraws  % Loop through draws
    ...
    theta_old = [theta_old; C(3*Nc+1:4*Nc, 1)];
    ...
    C_new(3*Nc+1:4*Nc,1) = theta_new(Nc+1:2*Nc);
    C_new(4*Nc+1, 2) = theta_new(1);
